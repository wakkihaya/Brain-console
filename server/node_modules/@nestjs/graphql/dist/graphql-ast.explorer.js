...
let GraphQLAstExplorer = class GraphQLAstExplorer {
    ...
    getFieldTypeDefinition(typeNode, options) {
        ...
        if (isArray) {
            const { type: arrayType, required: arrayTypeRequired, } = this.getNestedType(lodash_1.get(type, 'type'));
            const typeName = this.addSymbolIfRoot(lodash_1.get(arrayType, 'name.value'));
        ...
        const typeName = this.addSymbolIfRoot(lodash_1.get(type, 'name.value'));
        return {
            name: required
                ? this.getType(typeName, options)
                : `Nullable<${this.getType(typeName, options)}>`,
